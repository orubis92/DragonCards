...
interface Fx { id: number; evento: GameState['ultimoEvento']; chi: PlayerId }
...
export function Partita({ config, onFine, onEsci }: Props) {
  ...
  const esegui = useCallback((azione: Action, chi: PlayerId) => {
    setState((s) => {
      ...
      if (ns.ultimoEvento) {
        fxId.current += 1;
        setFx({ id: fxId.current, evento: ns.ultimoEvento, chi });
        if (config.suoni) suona(ns.ultimoEvento.tipo === 'attacco' ? 'attacco' : ns.ultimoEvento.tipo === 'incantesimo' ? 'incantesimo' : ns.ultimoEvento.tipo === 'evoca' ? 'evoca' : 'turno');
      }
      return ns;
    });
    setSel(null);
  ...
  return (
    <div className={`partita ${sel ? 'in-selezione' : ''}`} onClick={() => sel && setSel(null)}>
      {/* ---- Avversario ---- */}
      <header className="barra barra-ia">
        <button className="btn-icona" onClick={(e) => { e.stopPropagation(); onEsci(); }} title="Abbandona">✕</button>
        <Ritratto nome={ia.nome} vita={ia.vita} cristalli={ia.cristalli} cristalliMax={ia.cristalliMax} mano={ia.mano.length} mazzo={ia.mazzo.length}
        ...
        <div className="mano-ia" aria-label={`${ia.mano.length} carte in mano`}>
      ...
      <section className="campo campo-ia">
        {ia.campo.map((d) => {
          ...
          return (
            <div key={d.uid} className={`slot ${classiFx(fx, d.uid, IA)}`} onClick={(e) => { e.stopPropagation(); tapDragoIA(d.uid); }}>
      ...
      <section className="campo campo-io">
        {io.campo.map((d) => (
          <div key={d.uid} className={`slot ${classiFx(fx, d.uid, IO)} ${sel?.tipo === 'campo' && sel.uid === d.uid ? 'sel' : ''}`}
            onClick={(e) => { e.stopPropagation(); tapMioDrago(d.uid); }}>
      ...
      <footer className="barra barra-io" onClick={(e) => e.stopPropagation()}>
        <Ritratto nome={io.nome} vita={io.vita} cristalli={io.cristalli} cristalliMax={io.cristalliMax} mano={io.mano.length} mazzo={io.mazzo.length}
      ...
      <section className="mano" onClick={(e) => e.stopPropagation()}>
        {io.mano.map((c) => {
      ...
      {sel?.tipo === 'mano' && defSel && (
        <div className="anteprima" onClick={(e) => e.stopPropagation()}>
...
function classiFx(fx: Fx | null, uid: number, lato: PlayerId): string {
  ...
  const e = fx.evento;
  const cls: string[] = [];
  if (e.tipo === 'attacco' && e.da === uid) cls.push(fx.chi === IO ? 'anim-attacca-su' : 'anim-attacca-giu');
...
interface RitrattoProps {
  ...
  attivo: boolean; bersagliabile?: boolean; colpito?: number; onClick?: (e: React.MouseEvent) => void;
}
function Ritratto({ nome, vita, cristalli, cristalliMax, mano, mazzo, attivo, bersagliabile, colpito, onClick }: RitrattoProps) {
  const pct = Math.max(0, Math.min(100, (vita / VITA_INIZIALE) * 100));
  return (
    <div className={`ritratto ${attivo ? 'attivo' : ''} ${bersagliabile ? 'bersagliabile' : ''} ${colpito ? 'anim-colpito' : ''}`} onClick={onClick}>
