...
import { azioniLegali, applica, other, puoAttaccare } from './rules';
...
export function valuta(state: GameState, pid: PlayerId, profondo = false): number {
  ...
  let s = 0;
  if (profondo) s -= rispostaAvversario(state, pid) * 0.7;
  // Vita: pesa di più quando è bassa (pericolo)
  s += vitaScore(io.vita) - vitaScore(avv.vita);
  // Campo
  for (const d of io.campo) s += valoreDrago(d);
  ...
  else if (io.vita - minaccia < 6) s -= (6 - (io.vita - minaccia)) * 2;
  return s;
}
...
function rispostaAvversario(state: GameState, pid: PlayerId): number {
  ...
  for (const a of avv.campo) {
    ...
    for (const d of bersagli) {
      ...
      const subito = a.scudo ? 0 : dannoElementale(d.attacco, vantaggio(d.elemento, a.elemento));
      let g = 0;
...
// ---------------------------------------------------------------------------
function scegliGreedy(state: GameState, rnd: () => number): Action {
  const pid = state.attivo;
  const azioni = azioniLegali(state);
  const base = valuta(state, pid);
  ...
  for (const a of azioni) {
    ...
    const s = valuta(applica(state, a), pid) + rnd() * 1.5; // un po' di rumore: non è perfetto
    if (s > bestScore) {
      bestScore = s;
      best = a;
    }
...
function scegliRicerca(state: GameState): Action {
  const pid = state.attivo;
  let fascio: Nodo[] = [{ state, prima: null, score: valuta(state, pid, true), finito: false }];
...
// API
// ---------------------------------------------------------------------------
export function scegliAzione(state: GameState, livello: Livello, rnd: () => number = Math.random): Action {
  ...
  switch (livello) {
    case 1: return scegliCasuale(state, rnd);
    case 2: return scegliGreedy(state, rnd);
    case 3: return scegliRicerca(state);
  }
}
